...
var
  ...
  expr1, expr2, expr3, expr4, expr5, expr6, expr7, expr8, expr9, expr10, expr11 : Extended;
  ...
  for pass := 1 to maxpass do begin
    ...
    if not Flip_YZ then begin
       ...
       if neutrino then begin
         ...
         delta := ( sqrt(4/5) * UnitaryCharge * Hhat ) / ( 2 * Pi * ElectronNeutrinoMass * SpeedOfLight * Permittivity );
         ...
         delta := ( sqrt(4/5) * UnitaryCharge * Hhat ) / ( 2 * Pi * NeutronMass * SpeedOfLight * Permittivity );
       ...
       for xpos:=0 to GridWidth-1 do begin {scan grid's x coords}
         ...
         for ypos:=0 to GridHeight-1 do begin {scan grid's y coords}
           for zpos:=0 to GridDepth-1 do begin {scan grid's z coords}
             ...
             ///
             ///
             term1:=delta/r_gamma; // delta/r_gamma?;
...
//             PrevVectorPotential:= points[scr]^[xpos,ypos,zpos].VectorPotential;
             ...
             if E_useFormula and not smoothing then begin
               ...
               with points[NewScreen]^[xpos,ypos,zpos].Electric do begin
                 ...
                 expr3 := (3*vect.x*r/Power(sqr(r),5/2));
                 expr4 := (3*vect.y*r/Power(sqr(r),5/2));
                 expr5 := MeC_Hhat*(3*vect.y*r/Power(sqr(r),2));
                 expr6 := sqr(MeC_Hhat)*(vect.x*r/Power(sqr(r),3/2));
                 expr7 := (3*vect.x*r/Power(sqr(r),2));
                 expr8 := (vect.y*r/Power(sqr(r),3/2));
                 expr9 := (3*vect.z*r/Power(sqr(r),5/2));
                 expr10 := MeC_Hhat*(3*vect.z*r/Power(sqr(r),2));
                 expr11 := (vect.z*r/Power(sqr(r),3/2));
                 ...
                 //
                 with points[NewScreen]^[xpos,ypos,zpos].Electric do begin
                     x:=-(expr3*sqr(actual_x) + expr5*sqr(actual_x) -
                        vect.x*r/Power(sqr(r),3/2) - expr6*sqr(actual_x) -
                        MeC_Hhat*(vect.y*r/sqr(r)) + expr4*actual_x*actual_y -
                        MeC_Hhat*expr7*actual_x*actual_y -
                        sqr(MeC_Hhat)*expr8*actual_x*actual_y - sqr(MeC_Hhat)*vect.x);

                     y:=-(expr3*actual_x*actual_y + expr5*actual_x*actual_y -
                        expr6*actual_x*actual_y + expr4*sqr(actual_y) -
                        MeC_Hhat*expr7*sqr(actual_y) - expr8 -
                        sqr(MeC_Hhat)*expr8*sqr(actual_y) + MeC_Hhat*(vect.x*r/sqr(r)) - sqr(MeC_Hhat)*vect.y);

                     z:=-(expr3*actual_x*actual_z + expr5*actual_x*actual_z -
                        expr6*actual_x*actual_z + expr4*actual_y*actual_z -
                        MeC_Hhat*expr7*actual_y*actual_z - sqr(MeC_Hhat)*expr8*actual_y*actual_z);
                 end;
               end;
             end;
