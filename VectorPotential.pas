...
//
//  (c) Copyright 1998 - 2024 : Declan Traill
//
...
const
  // These compile-time switches can be used to re-configure the software model
  // for different modeling requirements - mainly to do with allowing larger modeled
  // space (i.e. more data points) by reducing the amount of data stored for each point.
  // If these options are changed, the software must be recompiled for it to take effect.
  STANDING_WAVE_ANALYSIS=false;
  TWO_PARTICLE_COULOMB_FORCE=false;
  TWO_PARTICLE_REFLECTION_FIELDS=true;
  TWO_PROTON_MODEL=true;

  // Number of points to divide by in vector functions across point groups
  nP=3;

{$IF TWO_PARTICLE_COULOMB_FORCE}
  GRID_LIMIT=1000;
{$ELSE}
{$IF TWO_PROTON_MODEL}
  GRID_LIMIT=260;
...
{$IFEND}
  ...
  TForm1 = class(TForm)
    ...
    Start29: TRadioButton;
    Start30: TRadioButton;
    ...
    procedure start29Click(Sender: TObject);
    procedure start30Click(Sender: TObject);
  ...
  public
    ...
    function VectorCross(v1, v2: VectorDouble): VectorDouble;
    function PowerFlow(Apoint: point): VectorDouble;
    function RMS_PowerFlow(Apoint: point): VectorDouble;
    ...
    function VectorDoubleNull(vectDouble: VectorDouble): boolean;
...
{point in question are in range: 1=in range, 0=out of range}
  ...
  StartOptionCaptions : array[1..30] of String = (
                                  ('1: e'),
                                  ('2: p'),
                                  ('3: ee^^'),
                                  ('4: pp^^'),
...
const
  ...
  SpeedOfLight=2.99792458E8;   {Speed of Light in meters per second 2.99792458e8}
  ...
  Hhat=1.054571596E-34;   {reduced Planck's constant 1.054571596e-34}
  ...
  ElectronMass=9.1093835611E-31;   // (9.1093835611e-31 kg,  8.187105650638658873144497804e-14 Joules)
...
//  points2: TDynamicArray;
  ...
  particle1_Power: array of array of array of VectorDouble;
  particle2_Power: array of array of array of VectorDouble;
  ...
  Rate_Of_Time, New_RateOfTime: extended;
  ...
  AllFields: boolean;
  smoothing: boolean;
  two_particles, prev_two_particles: boolean;
...
//       EnergyCorrectionCheckBox.Checked := false;
  ...
  case StartOption of
   ...
   29: begin  // if Two Protons being modelled
      Start29.Checked:=true;
      end;

   30: begin  // if a Photon being modelled
      Start30.Checked:=true;
      end;
  end;
  ...
  CurrentScreen:=0;          {start by displaying bitmap1}
  DisplayScreen(CurrentScreen); {make it visible}
  New_DisplayLevel:=1995;    {default Level slider to (2000 - n)*1000%}
    ...
    //for scr:=0 to 1 do             {scan both copies of the grid's points}
      for i:=0 to (GridWidth-1) do     {and set all values to zero}
        for j:=0 to (GridHeight-1) do
          for k:=0 to (GridDepth-1) do begin
            ...
            particle1_Power[i,j,k]:=NullVectDouble;
            particle2_Power[i,j,k]:=NullVectDouble;
          end;
...
var
  ...
  VectGrp: VectorGrp;
  dist: longint;
  orthogonal_particles, particles_end_to_end: boolean;
  particle1_spin, particle2_spin, saved_sign, I: integer;
  ...
  c1,c2,c3,A,DistFromCenter,ExpectedAccel,E_Energy_Sum,B_Energy_Sum, M, MeC_Hhat: Extended;
  sine_wave_sum, neutrino_quarklets, field_stats: boolean;
  progress, progress_inc, r_contracted, x_contracted, x_velocity, gamma, r_gamma : single;
  expr1, expr2 : Extended;
  Qsin1, Qcos1, Qsin2, Qcos2, c, E0, Q: extended;
  charge_sign, charge_factor: single;
...
begin
  ...
  case StartOption of
   ...
   29: begin  // if two protons being modeled
       electron:=false;
       positron:=false;
       proton:=true;
       r_lower_limit:=ProtonComptonRadius;
  ...
  if StartOptionChanged then begin
    ...
    // If changing between single and two-particle modeling,
    // set the DistBetween variable initial value correctly.
    if not (two_particles = prev_two_particles) then begin
      if not two_particles then begin
        DistBetween.Text:='0';
      end
      else begin
        if proton then
          DistBetween.Text:='100'
        else
          DistBetween.Text:='50';
      end;
      PercentBetweenParticles:=strtofloat(DistBetween.Text);
      prev_two_particles := two_particles;
    end;

    if (proton or neutron or quark_up or quark_down) then begin
      if proton and two_particles then
        ActualGridWidth.Text:=FloatToStrf(3.5E-14,ffExponent,5,2) {display actual size in metres that grid represents}
      else
  ...
  for pass := 1 to maxpass do begin
    ...
    if not Flip_YZ then begin
       ...
       if neutrino then begin
         ...
         delta := ( sqrt(4/5) * UnitaryCharge * Hhat ) / ( 4 * Pi * ElectronNeutrinoMass * SpeedOfLight * Permittivity );
         ...
         delta := ( ProtonCharge * Hhat ) / ( 4 * Pi * ProtonMass * SpeedOfLight * Permittivity );
         ...
         delta := ( sqrt(4/5) * UnitaryCharge * Hhat ) / ( 4 * Pi * NeutronMass * SpeedOfLight * Permittivity );
         ...
         SpinConstant:=( Hhat / ProtonMass ); // Metres^2/(Radians*Second)
         delta := ( (2/3)*ProtonCharge * Hhat ) / ( 4 * Pi * ProtonMass * SpeedOfLight * Permittivity );
         ...
         SpinConstant:=( Hhat / ProtonMass ); // Metres^2/(Radians*Second)
         delta := ( (1/3)*ProtonCharge * Hhat ) / ( 4 * Pi * ProtonMass * SpeedOfLight * Permittivity );
         ...
         delta := ( ElectronCharge * Hhat ) / ( 4 * Pi * ElectronMass * SpeedOfLight * Permittivity );
...

{
       ...
       //
       // Ye = ((Qe*Hhat) / (4*Pi*r*i*Me*c*Eo )) * Exp( ( - i * Me * c^2 / Hhat ) * ( T – r/c ) )
       //
       // and the Electric Potential div(psi) in spherical coordinates is
       //
       // V = (Qe / ( 4 * Pi * r * Me * c * Eo )) * (Me * c * r – i * Hhat) *  Exp( ( - i * Me * c^2 / Hhat ) * ( T – r/c ) )
       //
       // Where:
       ...
       // term3:=-sin(theta);
       //
       // Assign values to x, y, z coordinates.
       //   x:=term1 * term2;
       //   y:=term1 * term3;
       //   z:=0;
         ...
         for ypos:=0 to GridHeight-1 do begin {scan grid's y coords}
           for zpos:=0 to GridDepth-1 do begin {scan grid's z coords}
             ...
             if two_particles then begin
               ...
               if (strtofloat(DistBetween.Text) = 100) then
                 DistBetween.Text := '99';
               ...
               // The x velocity component fraction is the absolute value of Sin(theta_xy),
               // where theta_xy is the angle (at the origin) in a triangle formed from
               // the x and y coorinate locations.
               if y = 0 then
                 theta_xy:=Pi/2
               else
                 theta_xy:=arctan(x/y);

               //x_velocity := sqrt(sqr(x*dx)/sqr(r))*x_sign*velocity;   // old formulation
               x_velocity := abs(sin(theta_xy))*x_sign*velocity;
             end;
             ...
             case StartOption of
               ...
               3: begin  // if two electrons being modeled
                   theta1:=abs(particle1_spin)*theta_const*(Time - r1/SpeedOfLight);
                   theta2:=abs(particle2_spin)*theta_const*(Time - r2/SpeedOfLight);
                   term1a:=delta/r1;
                   term1b:=delta/r2;
               end;
               ...
               29: begin  // if two protons being modeled
                   // Note: Grid size of 200x200x200 gives a good Acceleration calculation match.
                   theta1:=abs(particle1_spin)*theta_const*(Time + r1/SpeedOfLight);
                   theta2:=abs(particle2_spin)*theta_const*(Time + r2/SpeedOfLight);
                   term1a:=sign(ProtonCharge) * abs(delta)/r1;
                   term1b:=sign(ProtonCharge) * abs(delta)/r2;
               end;
             ...
             with points[NewScreen]^[xpos,ypos,zpos].PsiVect do begin
               if two_particles then begin
                 if orthogonal_particles then begin
                   if particle2_spin > 0 then begin
                     // x1' = x1
                     // y1' = y1
                     // z1' = z1
                     // x2' = z2
               ...
               if (smoothing) then begin
                 ...
                 if two_particles then begin
                   ...
                   if (pass=1) or (pass=3) then begin
                     ElectricPotential:=ElectricPotential + abs(particle1_spin)*Q/(4*Pi*r1*Permittivity)
                   end;

                   if (pass=2) or (pass=3) then begin
                     ElectricPotential:=ElectricPotential + abs(particle2_spin)*Q/(4*Pi*r2*Permittivity)
                   end;
                 end
                 else begin
                   // get actual distance in metres
                   ...
                   ElectricPotential:=Q/(4*Pi*r*Permittivity)
                 end;
               end
               else begin // if not smoothing...
...
{$IF NOT TWO_PARTICLE_COULOMB_FORCE}
             ...
             // Note: Electric field is NOT based on the Vector Laplacian of Psi:
			       //       which is Grad(Div(psi)) - Curl(Curl(psi))
             //
             //	The Electric field is -Div(Grad()) on each x,y,z term of the ElectricPotential minus dA/dt.
             // (will add the rest of the Electric field definition once the Vector Potential is known)
			       //
			       // Note: the VectDiv() and ScalarGrad() functions used here are mathematically identical and both
			       // calculate d/dx, d/dy, d/dz on the vector or scalar passed into them, so given that psi is a vector
			       // (or complex scalar) and the Electric potential is a scalar and the Electric field is a vector,
			       // the following is true:
             // E = -ScalarGrad(VectDiv(Psi)) - dA/dt
             ...
             if not photon then with points[NewScreen]^[xpos,ypos,zpos].VectorPotential do begin
               if (smoothing) then begin
                 ...
                 x := ((M/Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.y);
                 y := -((M/Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.x);
                 z := ((M/Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.z);
               end;
             end;
             ...
             if not E_useFormula or smoothing then begin
               if (smoothing) then begin
                 ...
                 ElecFieldFromA.x := -sqr(M * SpeedOfLight / Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.x;
                 ElecFieldFromA.y := -sqr(M * SpeedOfLight / Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.y;
                 ElecFieldFromA.z := -sqr(M * SpeedOfLight / Hhat)*points[NewScreen]^[xpos,ypos,zpos].PsiVect.z;
               end;
...
{$IFEND}
             ...
             if E_useFormula and not smoothing then begin
               ...
               // Part 1:
               // Flip the Maple calculations orientation to match the model,
               // such that the vector arrows of the E field are the same when switching between useFormula true/false.
               actual_x:=-actual_x;
               actual_y:=-actual_y;
               ...
               with points[NewScreen]^[xpos,ypos,zpos].Electric do begin
                 ...
                 //
                 x:=(3*(vect.x/Power(r,4))*sqr(actual_x) -
                    3*(M*c/Hhat)*(vect.y/Power(r,3))*sqr(actual_x) -
                    vect.x/Power(r,2) -
                    sqr(M*c/Hhat)*(vect.x/Power(r,2))*sqr(actual_x) +
                    (M*c/Hhat)*(vect.y/r) +
                    3*(vect.y/Power(r,4))*actual_x*actual_y +
                    3*(M*c/Hhat)*(vect.x/Power(r,3))*actual_x*actual_y -
                    sqr(M*c/Hhat)*(vect.y/Power(r,2))*actual_x*actual_y -
                    sqr((M*c/Hhat))*vect.x);

                 y:=(-3*(vect.x/Power(r,4))*actual_x*actual_y +
                    3*(M*c/Hhat)*(vect.y/Power(r,3))*actual_x*actual_y +
                    sqr((M*c/Hhat))*(vect.x/Power(r,2))*actual_x*actual_y +
                    3*(vect.y/Power(r,4))*sqr(actual_y) +
                    3*(M*c/Hhat)*(vect.x/Power(r,3))*sqr(actual_y) -
                    (vect.y/Power(r,2)) -
                    sqr(M*c/Hhat)*(vect.y/Power(r,2))*sqr(actual_y) -
                    (M*c/Hhat)*(vect.x/r) -
                    sqr((M*c/Hhat))*vect.y);

                 z:=(3*(vect.x/Power(r,4))*actual_x*actual_z +
                    3*(M*c/Hhat)*(vect.y/Power(r,3))*actual_x*actual_z -
                    sqr(M*c/Hhat)*(vect.x/Power(r,2))*actual_x*actual_z +
                    3*(vect.y/Power(r,4))*actual_y*actual_z -
                    3*(M*c/Hhat)*(vect.x/Power(r,3))*actual_y*actual_z -
                    sqr(M*c/Hhat)*(vect.y/Power(r,2))*actual_y*actual_z);
             ...
             with points[NewScreen]^[xpos,ypos,zpos].Magnetic do begin
               ...
               if H_useFormula and not smoothing then begin
                 ...
                 if (pass=3) then begin
                   ...
                   expr1 := (M/(Hhat*sqr(r)));
                   expr2 := (sqr(M)*SpeedOfLight/(r*sqr(Hhat)));

                   // Part 1:
                   // Flip the Maple calculations orientation to match the model,
                   // such that the vector arrows of the B field are the same when switching between useFormula true/false.
                   actual_y:=-actual_y;
                   actual_x:=-actual_x;
                   ...
                   x:=(expr1*vect.x*actual_z + expr2*vect.y*actual_z);
                   y:=(expr1*vect.y*actual_z - expr2*vect.x*actual_z);
                   z:=(-expr1*vect.x*actual_x - expr2*vect.y*actual_x -
                        expr1*vect.y*actual_y + expr2*vect.x*actual_y);
                 ...
                 // if the second particle is a positron or proton, we must reverse the direction of the magnetic field vector, as
                 // the above equation is for an negatively charged particle such as an electron.
...
{$IFEND}
       ...
       if EnergyCorrection then begin
         ...
         // account for Relativistic mass increase if the particle is moving
         EnergyFactor := EnergyFactor * gamma;
...
{$IF TWO_PARTICLE_REFLECTION_FIELDS}
 ...
 {$IFEND}
          Power_x1:=particle1_Power[xpos, ypos, zpos].x;  //  The RMS Power flow from particle 1
          Power_x2:=particle2_Power[xpos, ypos, zpos].x;  //  The RMS Power flow from particle 2
          ...
          // If:
          //
          // (1) two oppositely charged particles, reverse the sign (charge) of one particle in order to calculate the
          ...
          //
          // (2) or two positively charged particles, reverse the sign of the power as the repulsion is mainly coming from
          // outside the two particles (which is hard to model well given the space constrainst of the model). So model it as
          // a repulsion from in between the particles.
          if ((electron and positron) or (not electron and (positron or proton))) then Power_x2:=-Power_x2;
          ...
          // This line (and the 'end;' below) can be uncommented so that only grid points within the sphere
          // from the center of particle 1, out to the center of particle 2 are summed and counted in the total.
          //
          //if (sqrt(sqr(xpos - particle1_x) + sqr(ypos - midy) + sqr(zpos - midz)) <= p1_p2_diff) then begin

          // if oppositely charged particles, now reverse the force, as the opposite charges were converted into
    ...
    // other particle's center represents all of the contributing grid points to the total force attributed to the central point.
    // Then inverse law applies from the particle center to the midpoint between the two particles (where they started).
    //
    ...
    // The actual pressure at a single, central grid point, where r is the number of grid points between the two
    // particle centers.
    //
    // (4/3)πr^3/(r/2) is the number of grid points in the sphere extending from the particle center,
    // to the other particle's center, multiplied by the number of grid points along the line that
...
//      EnergyCorrectionCheckBox.Checked:=false;
    ...
    Auto_Scale(CurrentScreen);    {determine the scaling factor required for current data}
    UpdateDetails;                {ensure the screen is displaying up-to-date info}
    UpdateBitmap(CurrentScreen);  {Redraw the new data on the current screen(bitmap)}
    DisplayScreen(CurrentScreen); {Display the current screen}
  end;
  ...
  if DoUpdate then begin
    ...
    if (not FreezeTime) or Flip_YZ then begin   {if time is running}
      ...
      RecalcFields(CurrentScreen);

      if CurrentScreen=0 then CurrentScreen:=1 else CurrentScreen:=0;  {initiate screen swap}
      ReDraw:=true;
    end;
...
var
  ...
  pixel_count,black_count,white_count: longint;
  is_black,is_white: boolean;
  black_thres, white_thres: longint;
...
repeat
  ...
  repeat                                {all Z values are drawn, one at a time }
    ...
    repeat    {scan through each (x,y) in Z plane of grid}
      ...
      repeat
        // Flip the Y/Z plane (rotate the top toward the back - so we are viewing from the side)
        if not ViewTop then begin
           swap := j;
           j := k;
           k := GridDepth - swap - 1;
        end;
...
{$ELSE}
          POWER_FLOW_FIELD: begin {Show Electric, Magnetic, Power flow Fields}
            VectorType:=true;
          end;
{$IFEND}
{$IFEND}
...
{$IFEND}
        ...
        if VectorType then begin {convert component values to colour values}
          ...
          with ColVect do begin   {Combine colours}
            ...
            if ( X_RGB = GREY ) then begin
              xyzcol:=ColourRange(xx/3,Amplification);   {set equal RGB colour values}
              colour:=colour + xyzcol*RED+xyzcol*GREEN+xyzcol*BLUE;    {Combine colours}
              if xyzcol >= black_thres then
                is_black:=false;
              if xyzcol >= white_thres then
                is_white:=true;
            end else begin
              colour:=colour + Round2(abs(xx))*X_RGB;
              if Round2(abs(xx)) >= black_thres then
                is_black:=false;
              if Round2(abs(xx)) >= white_thres then
                is_white:=true;
            end;

            if ( Y_RGB = GREY ) then begin
              xyzcol:=ColourRange(yy/3,Amplification);   {set equal RGB colour values}
              colour:=colour + xyzcol*RED+xyzcol*GREEN+xyzcol*BLUE;    {Combine colours}
              if xyzcol >= black_thres then
                is_black:=false;
              if xyzcol >= white_thres then
              ...
              colour:=colour + Round2(abs(yy))*Y_RGB;
              if Round2(abs(yy)) >= black_thres then
                is_black:=false;
              if Round2(abs(yy)) >= white_thres then
                is_white:=true;
            end;

            if ( Z_RGB = GREY ) then begin
              xyzcol:=ColourRange(zz/3,Amplification);   {set equal RGB colour values}
              colour:=colour + xyzcol*RED+xyzcol*GREEN+xyzcol*BLUE;    {Combine colours}
              if xyzcol >= black_thres then
                is_black:=false;
              if xyzcol >= white_thres then
              ...
              colour:=colour + Round2(abs(zz))*Z_RGB;
               if Round2(abs(zz)) >= black_thres then
                is_black:=false;
               if Round2(abs(zz)) >= white_thres then
                is_white:=true;
            end;

            if ViewTop then begin
              SignArray[i,j,0]:=Sign(xx);
              SignArray[i,j,1]:=Sign(yy);
              SignArray[i,j,2]:=Sign(zz);
            end
            else begin
              SignArray[i,k,0]:=Sign(xx);
              SignArray[i,k,1]:=Sign(yy);
              SignArray[i,k,2]:=Sign(zz);
            end;
          end;
        end;
        ...
        if (not VectorType) or IsGrey then begin {if greyscale display required}
          ...
          if ViewTop then begin
            ...
            SignArray[i,k,0]:=Sign(xyzcol);
            SignArray[i,k,1]:=Sign(xyzcol);
            SignArray[i,k,2]:=Sign(xyzcol);
          end;
          is_black:=false;
          is_white:=false;
          if xyzcol < black_thres then
        ...
        if Rendered then begin {if colour or grey rendered display req'd}
          if ViewTop then begin
            ...
            ColourArray[i,k]:=colour;   {save point's colour in array}
          end;
          Inc(pixel_count,1);
          if is_black then
            Inc(black_count,1);
            ...
            repeat
              ...
              with ArrowVect do begin
                if ViewTop then begin
                  ...
                  yy:=y;
                  x:=x*ReScaleFactor*ArrowScale;          {re-scale raw values}
                  y:=-z*ReScaleFactor*ArrowScale;
                  z:=yy*ReScaleFactor*ArrowScale;
                end;
              end;
          ...
          repeat
            ...
            repeat
              ...
              if ViewTop then begin
                ...
                jk := GridDepth - Ytop - 2;
                p1:=points[scr]^[Xleft,k,jk];
                if Xleft<(GridWidth-1) then      {get relevant point vectors}
                  p2:=points[scr]^[Xleft+1,k,jk] else p2:=NullPoint;
                if YTop<(GridHeight-1) then
                  p3:=points[scr]^[Xleft,k,jk+1] else p3:=NullPoint;
                if (Xleft<(GridWidth-1)) and (YTop<(GridHeight-1)) then
                  p4:=points[scr]^[Xleft+1,k,jk+1] else p4:=NullPoint;
              end;
...
until (not (save_frames and save_3D)) or (k>GridDepth-1); {only repeat if tiling req'd and Z plane in range}
  ...
  if (AutoScale=CONTINUAL) or (AutoScale=AVERAGE) then begin
    ...
    if (white_count/pixel_count) > 0.6 then begin
      if DisplayLevel.Position > 0 then
        DisplayLevel.Position:=2000 - Trunc((2000 - DisplayLevel.Position)/2);
      New_DisplayLevel:=DisplayLevel.Position;
      DoUpdate:=true;
    end;
  end;
...
begin
  ...
  if (PercentBetweenParticles <> newPercentage) then begin
    PercentBetweenParticles := newPercentage;
    DoUpdate:=true;
    Restart:=true;
  end;
...
procedure TForm1.start29Click(Sender: TObject);
{The 29th Start option has been selected}
begin
  New_StartOption:=29;
  if StartOption<>New_StartOption then begin
    DoUpdate:=true;
...
procedure TForm1.start30Click(Sender: TObject);
{The 30th Start option has been selected}
begin
  New_StartOption:=30;
  if StartOption<>New_StartOption then begin
    DoUpdate:=true;
...
//                   end;
      ...
      AVERAGE: begin
                   Auto4.Checked:=true;
                   AutoWarn.Top:=Auto4.Top+2; {Show Auto-scale warning indicator}
                   AutoWarn.Visible:=true;    {next to the relevant button.}
                   AutoWarnTimer.Enabled:=true;
                 end;
    end;
...
//  ActualGridWidth.Text:=FloatToStrf(ActualWidth,ffExponent,5,2); {display actual size in metres that grid represents}
  ...
  if (ArrowScaleFactor <> New_ArrowScaleFactor) or FirstPass then begin
    ArrowScaleFactor := New_ArrowScaleFactor/10;
    ReDraw:=true;                      {Trigger a screen re-draw}
  end;
...
var
  amp: Extended;
begin
  amp:= sqrt(sqr(E_vect.x)/2 + sqr(E_vect.y)/2 + sqr(E_vect.z)/2);
  Result:=0.5*Permittivity*sqr(amp);
end;
...
begin
  H := BtoH(B_vect);
  amp:= sqrt(sqr(H.x)/2 + sqr(H.y)/2 + sqr(H.z)/2);
  Result:=0.5*Permeability*sqr(amp);
end;

function TForm1.PowerFlow(Apoint: point): VectorDouble;
...
function TForm1.RMS_PowerFlow(Apoint: point): VectorDouble;
{Calculate the RMS of Power flow vector at a point in the grid by taking }
{the vector cross product of the RMS Electric & Magnetic fields, and  }
{dividing the result by the Permeability of free space.}
var
  vect, E, B: VectorDouble;
begin
  ...
  with Apoint do begin
    E.x := Electric.x/sqrt(2);
    E.y := Electric.y/sqrt(2);
    E.z := Electric.z/sqrt(2);
    B.x := Magnetic.x/sqrt(2);
    B.y := Magnetic.y/sqrt(2);
    B.z := Magnetic.z/sqrt(2);
    vect:=VectorCross(E,B);
  end;
...
var
  Vect: Vector;
  VectDouble: VectorDouble;
begin
  Vect:=NullVect;
  VectDouble:=NullVectDouble;
  with Apoint do
    case Field of   {depending on which field is required to display}
      ...
      POWER_FLOW_FIELD: VectDouble:=PowerFlow(Apoint);       {Show Power flow}
{$IF NOT TWO_PARTICLE_COULOMB_FORCE}
...
{$IFEND}
  ...
  // proton power calculation exceed the range of a single, so use a double then put a ceiling on it of 1E30 so it fits into s single
  if (Field = POWER_FLOW_FIELD) then begin
    if (VectDouble.x > 1E30) or (VectDouble.x < -1E30) then Vect.x := VectDouble.x / 1E10 else Vect.x := VectDouble.x;
    if (VectDouble.y > 1E30) or (VectDouble.y < -1E30) then Vect.y := VectDouble.y / 1E10 else Vect.y := VectDouble.y;
    if (VectDouble.z > 1E30) or (VectDouble.z < -1E30) then Vect.z := VectDouble.z / 1E10 else Vect.z := VectDouble.z;
  end;
...
begin
  with Apoint do begin
    case DisplayField of
      ...
      POWER_FLOW_FIELD:       isNull:=VectorDoubleNull(PowerFlow(Apoint));
{$IF NOT TWO_PARTICLE_COULOMB_FORCE}
...
function TForm1.VectorDoubleNull(vectDouble: VectorDouble): boolean;
{Look at each component vector to see if the vector is overall a}
{Null vector.}
begin
  with vectDouble do
...
begin
  ...
  if MaintainAspect then          {if grid aspect ratio is to be preserved,}
    if ScreenAspect>Aspect then   {then adjust scaling values to allow}
      ScrScaleY:=ScrScaleY*Aspect {the largest image with the same aspect ratio}
    else
...
begin
  ...
  if GridHeight = 0 then begin
    GridHeight := New_GridHeight;
  end;

  if GridDepth = 0 then begin
    GridDepth := New_GridDepth;
  end;

  PPMx:=New_GridWidth/newWidth;
  PPMy:=New_GridHeight/(newWidth*GridHeight/GridWidth);
  PPMz:=New_GridDepth/(newWidth*GridDepth/GridWidth);
  ...
  PointArea:=dy*dz;             {Area each pixel represents}
...
function TForm1.VectorCross(v1, v2: VectorDouble): VectorDouble;
var
  ResultVect: VectorDouble;
begin
  with ResultVect do begin
...
{$IFEND}
{$ELSE}
         POWER_FLOW_FIELD: begin {calc Electric, Magnetic, Power flow Fields}
            Vect:=VectorProperty(Field,points[scr]^[i,j,k]);
            VectorType:=true;
         end;
{$IFEND}
